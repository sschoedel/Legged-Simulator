...
void MeshPool::setup(ModelConfig *_config)
{
	...
	mGlslShadowFloor = gl::GlslProg::create(loadAsset("shadow_shader_floor.vert"), loadAsset("shadow_shader_floor.frag"));
}
void  MeshPool::clear() 
{
...
MeshRef MeshPool::getMesh(std::string name)
{
	...
	if (name == "floor") { m->setup(name, geom::Plane().size(vec2(10000,10000)), vec3(0, 0, 0),Color(0.8,0.8,0.82), mGlslShadowFloor); };
	if (name == "body") { m->setup(name, geom::Capsule().radius(35).length(100).direction(vec3(1, 0, 0)), vec3(0, 0, 0), Color(0.42, 0.576, 0.624), mGlslShadow); };
